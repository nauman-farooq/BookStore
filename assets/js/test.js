...
function updateCartItemCount() {
  ...
  if (cartItemCount) {
    var totalCount = addedItems.length;
    cartItemCount.textContent = totalCount === 1 ? totalCount + ' item' : totalCount + ' items';
  }
}
/*The code you provided now includes a duplicate check to prevent adding the same item multiple times to the cart. Additionally, I have rearranged the functions `updateTotalPrice` and `updateCartItemCount` to separate them from the event handlers.
Please note that in this code, the item duplication prevention is based on the `title` of the item. If you have items with the same title but different attributes, you may need to modify the duplicate check logic accordingly.
